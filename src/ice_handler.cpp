...
int ICEHandler::DecodeStunBindingAttributesMsg (const char* pBuff, int Len)
{
    ...
    {
        const STUN_MSG_COMMON* pMsgComm        = reinterpret_cast<const STUN_MSG_COMMON*>(pAttributesMsg);
        ...
        tylog("AttributeType=%hu, attr len=%d, pData=%p", AttributeType, AttributeLen, pData);
        ...
        switch (AttributeType)
        {
            ...
            case ATTRIBUTE_USER_NAME:
            {
                ...
                std::string username(pData, pData + std::min(kEnoughLen, AttributeLen));
                tylog("ice username=%s", username.data()); // taylor last char is '\0' ?
            ...
            case ATTRIBUTE_CONTROLLING:
            {
                ...
                std::string controlling(pData, pData + 8);
                break;
            }
            ...
            case ATTRIBUTE_MESSGAE_INTEGRITY:
            {
                ...
                if (20 != AttributeLen) {
                    // taylor m_LogStr << " MESSGAE_INTEGRITY:    Len=" << AttributeLen << " Err\n";
                } else {
                    std::string Hmac(pData, pData + 20);
                }
            ...
            case ATTRIBUTE_FINGERPRINT:
            {
                ...
                std::string Fingerprint(pData, pData + 4);
                break;
            }
...
{
    STUN_XOR_MSG_MAPPED_V4_ADDRESS& pMapAddr = *(STUN_XOR_MSG_MAPPED_V4_ADDRESS*) pBuff;
    pMapAddr.AttributeType  = htons (ATTRIBUTE_XOR_MAPPED_ADDR);
    pMapAddr.Len            = htons (STUN_IPV4_ADDR_LEN);
    pMapAddr.Reserved       = 0;
    pMapAddr.ProtocolFamily = 0x01;
    pMapAddr.Port = htons (myport) ^ htons (STUN_MAGIC >> 16);;
    pMapAddr.Ip   = inet_addr(myip.data()) ^ htonl (STUN_MAGIC);

    // send to client pSrcAddr, 
    // taylor struct sockaddr_in* pSrcAddr = g_pConfig->stCmdContext.pstClientAddr;
    return (int) sizeof (STUN_XOR_MSG_MAPPED_V4_ADDRESS);
}

// taylor param type const?
int ICEHandler::EncoderMsgIntergrity (const char* pMsgIntergrityBuff, int LeftLen, char* pHeadBuf, int Len)
{
    WEB_ERROR_CHECK ( (NULL == pMsgIntergrityBuff), -1, "pBuff is null");
    WEB_ERROR_CHECK ( ( (int) sizeof (STUN_MSG_INTEGRITY) > LeftLen), -2, "LeftLen[%d] err", LeftLen);
    ...
    HMAC_CTX     ctx;
    HMAC_CTX_init (&ctx);
    HMAC_Init_ex (&ctx, (unsigned char*) iceInfo_.LocalPassWord, sizeof iceInfo_.LocalPassWord, EVP_sha1(), NULL);
    HMAC_Update (&ctx, (unsigned char*) pHeadBuf, Len);
    HMAC_Final (&ctx, (unsigned char*) (pMsgIntergrity->HmacSha1), &Outlen);
    HMAC_CTX_cleanup (&ctx);
    return (int) sizeof (STUN_MSG_INTEGRITY);
}
...
unsigned int Crc32Update(CRC32_CTX *ctx, const unsigned char *data, int nbytes)
{
...
unsigned int Crc32Final(CRC32_CTX *ctx)
{
    return ctx->CrcState;
}
...
int ICEHandler::EncoderFingerprint (const char* pFingerprintBuff, int FingerprintLen, char* pHeadBuf, int Len)
{
    WEB_ERROR_CHECK ( (NULL == pFingerprintBuff), -1, "pBuff is null");
    WEB_ERROR_CHECK ( ( (int) sizeof (STUN_MSG_FINGERPRINT) > FingerprintLen), -2, "Len[%d] err", FingerprintLen);
    STUN_MSG_FINGERPRINT* pMsgIntergrity = (STUN_MSG_FINGERPRINT*) pFingerprintBuff;
...
int ICEHandler::HandleBindReq (const std::vector<char> &vBufReceive)
{
    ...
    if (m_pStunUseCandidate && EnumStateMachine::ICE_DONE > belongingPeerConnection_.stateMachine_)
    {
    ...
    std::copy(pHeadReq->TsxId, pHeadReq->TsxId + STUN_MSG_TRANSACTION_ID_LEN, pHeadRes->TsxId);
    ...
    ssize_t sendtoLen = sendto(g_sock_fd, SndBuff, SndLen, 0, reinterpret_cast<struct sockaddr *>(&g_stConnAddr), sizeof(struct sockaddr_in));
    if (-1 == sendtoLen) {
...
int ICEHandler::HandleIcePacket (const std::vector<char> &vBufReceive)
{
    ...
    const STUN_MSG_HEAD* pHead = reinterpret_cast<const STUN_MSG_HEAD*>(vBufReceive.data());

    if (EnumStateMachine::GET_CANDIDATE_DONE > belongingPeerConnection_.stateMachine_)
    {
        return -1;
    }
