...
#define VIDEO_MAX_SLIC_BUFF_LEN (2048000)
  ...
  int cts = 0;
  for (const std::shared_ptr<Extension> &ext : h.extensions) {
...
bool H264Unpacketizer::ParseStapAStartOffsets(const uint8_t *nalu_ptr,
                                              size_t length_remaining,
                                              std::vector<size_t> *offsets) {
  size_t offset = 0;
  while (length_remaining > 0) {
    // Buffer doesn't contain room for additional nalu length.
    if (length_remaining < sizeof(uint16_t)) return false;
    uint16_t nalu_size = ((nalu_ptr[0] << 8) | nalu_ptr[1]);
    nalu_ptr += sizeof(uint16_t);
    length_remaining -= sizeof(uint16_t);
    if (nalu_size > length_remaining) return false;
    nalu_ptr += nalu_size;
...
int H264Unpacketizer::ParseStapAOrSingleNalu(
  ...
  const auto &payload = packet->GetBuffer()->data() + header.header_length;
  auto length = packet->GetLength() - header.header_length;
  const uint8_t *nalu_start = payload + kNalHeaderSize;
  ...
  if (frame_buffer_.frames.empty()) {
    frame_buffer_.NewFrame(header);
  } else {
    int idx = (int)frame_buffer_.frames.size() - 1;
    if (frame_buffer_.slice_info[idx].timestamp !=
        packet->GetTimestamp()) {  // just incase?
      frame_buffer_.NewFrame(header);
    }
  }
...
int H264Unpacketizer::ParseFuaNalu(const std::vector<char> &vBufReceive) {
  ...
  if (first_fragment) {
    int idx = (int)frame_buffer_.frames.size() - 1;
    if (idx < 0 ||
        (frame_buffer_.slice_info[idx].timestamp != packet->GetTimestamp())) {
      frame_buffer_.NewFrame(header);
  ...
  auto &info = frame_buffer_.slice_info[idx];
  if (info.timestamp != packet->GetTimestamp()) {
    // maybe lost packet..
    tylor("Chn %" PRIu64
          ...
          length, header.padding_length, header.header_length - RTP_HEADER_LTH);
  }
  ...
  /*if ((payload[1] & kEBit) != 0) {
      frame_buffer_.PopSlices(slices);
  }*/

  return 0;
}

H264Unpacketizer::H264Unpacketizer(uint32_t ssrc)
    : Unpacketizer(ssrc, kMediaVideo) {
...
void H264Unpacketizer::Init() {
  ...
  unpack_params_.content_type = kVideoContentUnspecified;
...
int VideoGetExtInfo(unsigned char* pRtpData, int RtpLen)
{
    VIDEO_RTP_HEADER* pRtpHeader     = (VIDEO_RTP_HEADER*)pRtpData;
    int               CSRCOffset     = (pRtpHeader->bit4CC << 2);
    int               ExternLen      = 0;
    unsigned char*    pExtStart      = NULL;

    /*CSRC list是在ext数据之前属于RTPhead的一部分*/
    if (CSRCOffset + 12 > RtpLen)
    {
        return -1;
    }
    ...
    if (pRtpHeader->bit1X)
    {
        unsigned char* pBuff = pRtpData + CSRCOffset + sizeof(VIDEO_RTP_HEADER);
        VIDEO_RTP_EXT_HEADER* pRtpExtHeader = (VIDEO_RTP_EXT_HEADER*)pBuff;

        if (VIDEO_RFC5285_ONEBYTE_ORDER_NAME == ntohs(pRtpExtHeader->bit16Name))
        {
            ExternLen = (ntohs(pRtpExtHeader->bit16ExtLen) << 2);
            pExtStart = pBuff + sizeof(VIDEO_RTP_EXT_HEADER);
            unsigned char* pExtEnd = pExtStart + ExternLen;
            DecodeOneByteExt(pExtStart, pExtEnd);
        }

        ExternLen += (VIDEO_RTP_EXTERN_NAME_LEN + VIDEO_RTP_EXTERN_LEN_VALUE_LEN);
    }

    int RealOffset = ExternLen + CSRCOffset;
    return RealOffset;
}
...
std::vector<std::unique_ptr<MediaData>> H264Unpacketizer::Unpacketize(
    ...
    int                ExternOffset  = VideoGetExtInfo(pRtpData, RtpLen);
        int                ExternOffset  = VideoGetExtInfo(pRtpData, RtpLen);
  ...
  if ((len <= header.padding_length + header.header_length + 1)) {
    if (len != header.padding_length + header.header_length) {
      tylog("Chn %" PRIu64
            " RTP_Len receive a unknow rtp with padding: seq[%u] ssrc = %u, "
            "pt = %u rtp_len=%d padding_len=%d extern_offset=%d!",
            ssrc_, header.sequence_number, header.ssrc, header.payload_type,
            len, header.padding_length, header.header_length - RTP_HEADER_LTH);
    }
  ...
  auto payload = packet->GetBuffer()->data() + header.header_length;
  uint8_t nal_type = payload[0] & kH264TypeMask;
  if (nal_type == kH264FuA) {
    ParseFuaNalu(packet);
  } else {
    ParseStapAOrSingleNalu(packet);
  }
  if (header.marker) {
...
void H264Unpacketizer::UpdataSps(const uint8_t *data, size_t len) {
...
void H264Unpacketizer::UpdataPps(const uint8_t *data, size_t len) {
