...
#ifndef SRC_RTMP_RTMP_ASSIST_H_
#define SRC_RTMP_RTMP_ASSIST_H_
...
#define MAXC (MAX_CLIENT + START_CLIENT_IDX)
...
inline std::string RtmpStateToString(RTMP_STATE rtmpState) {
  switch (rtmpState) {
    case RTMP_STATE_INIT:
      return "INIT";
    case RTMP_STATE_TCP_CONNECTED:
      return "TCP_CONNECTED";
    case RTMP_STATE_RTMP_CONNECTED:
      return "RTMP_CONNECTED";
    case RTMP_STATE_PLAY:
      return "PLAY";
    case RTMP_STATE_PUBLISH:
      return "PUBLISH";
    default:
      return tylib::format_string("UnknownType[%d]", rtmpState);
  }
}
...
struct Client {
  ...
  int acceptFd = 0;
  RTMP rtmp{};  // librtmp ABI is disclosed
...
struct RTMP_CONFIG {
  ...
  std::map<pid_t, RTMP_PROCESS_INFO> m_WorkProcMap;  //监控进程使用
  int LogLevel;                                      //本地日志级别
...
class RtmpAssist {
 public:
  RtmpAssist();
  ~RtmpAssist();  // virtual ?
  int CleanupClient(Client* pClient);

  int HandleRtmpFd(int acceptFd);

  virtual PeerConnection* GetRtmpPeerPC() = 0;
 ...
 public:
  ...
  Client m_Clients[MAXC]{};
  RTMP& rtmp_ = m_Clients[0].rtmp;  // tmp
 ...
 private:
  ...
  int m_BframeStmNum;
  snowflake<1693561233000L> m_sessionidGenerator;
};
