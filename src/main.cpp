...
struct sockaddr_in g_stConnAddr; // to use data member
...
int HandleRequest()
{
    ...
    std::vector<char> vBufReceive(8 * 1024); // to use memory pool for so large buffer

    ssize_t iRecvLen = recvfrom(g_sock_fd, &vBufReceive[0], vBufReceive.size(), 0, (struct sockaddr *)&g_stConnAddr, (socklen_t *)&addr_size);
    ...
    tylog("taylor recv buffer data addr=%p, size=%zu", vBufReceive.data(), vBufReceive.size());
...
int main(int argc, char *argv[])
{
    ...
    ret = bind(g_sock_fd, reinterpret_cast<const sockaddr*>(&address), sizeof(address));
    assert(ret != -1);
    ...
    if (epoll_ctl(g_efd, EPOLL_CTL_ADD, g_sock_fd, &event) == -1) {
        tylog("epoll_ctl return -1, add g_sockfd=%d failed errno=%d[%s]", g_sock_fd, errno, strerror(errno));
        return 0;
    }
